...
- Read in `df_arrests`
- Use train_test_split to create two dataframes from `df_arrests`, the first is called `df_arrests_train` and the second is 
-- called `df_arrests_test`. Set test_size to 0.3, shuffle to be True. Stratify by the outcome  
...
- Initialize the GridSearchCV using the logistic regression model you initialized and parameter grid you created. Do 5 fold 
-- crossvalidation. Assign this to a variable called `gs_cv` 
- Run the model 
...
class LogisticRegression:
    def logistic_regression_model():
        ...
        # Splits training data and testing data
        ...
        gs_cv.fit(features_train, y_train)

        print(f'What was the optimal value for C? Did it have the most or least regularization? Or in the middle? {gs_cv.best_params_}')

        features_test = df_arrests_test[features]
        pred_lr = gs_cv.predict(features_test)
        
        # Creates 'pred_lr' column in df_arrests_test
        df_arrests_test['pred_lr'] = pred_lr

        df_arrests_train.to_csv('data/df_arrests_train.csv')
        df_arrests_test.to_csv('data/df_arrests_test.csv')
